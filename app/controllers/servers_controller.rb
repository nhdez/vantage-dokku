...
class ServersController < ApplicationController
  ...
  def test_connection
    ...
    render json: { success: false, message: "Failed to start connection test: #{e.message}" }
  end
  ...
  def update_server
    ...
    render json: { success: false, message: "Failed to start server update: #{e.message}" }
  end
  ...
  def install_dokku
    ...
    render json: { success: false, message: "Failed to start Dokku installation: #{e.message}" }
  end
  ...
  def restart_server
    ...
    if result[:success]
      ...
      render json: { success: false, message: result[:error], output: result[:output] }
    end
  rescue StandardError => e
    Rails.logger.error "Server restart failed: #{e.message}"
    render json: { success: false, message: "An unexpected error occurred: #{e.message}" }
  end

  def logs
    deployment_names = @server.deployments.pluck(:name)
    search_terms = [ @server.name, @server.display_name ] + deployment_names
    search_conditions = search_terms.map { "details ILIKE ?" }
