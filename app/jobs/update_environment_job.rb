class UpdateEnvironmentJob < ApplicationJob
  ...
  def perform(deployment_id, user_id, env_vars_params)
    ...
    ActionCable.server.broadcast("update_environment_#{@deployment.uuid}",
      type: "started", message: "Syncing environment variables for #{@deployment.dokku_app_name}...")

    begin
      # Start a transaction to ensure data consistency
      ActiveRecord::Base.transaction do
        # Only remove user-managed variables; preserve system-managed ones (e.g. DATABASE_URL)
        @deployment.environment_variables.user_managed.destroy_all
        ...
        # Sync environment variables to Dokku server (all vars including system-managed)
